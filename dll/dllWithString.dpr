...
function SayHello(const name: Pchar; const sizeName: Cardinal;
                  out response: PChar; out sizeResponse: Cardinal): boolean; stdcall;
var
  vResponse: string;
begin
  vResponse := 'Hello ' + name;
  response := Pchar(vResponse);
  sizeResponse := sizeof(response);
  result := true;
end;
